...
func apiStatus(clientset *kubernetes.Clientset) error {
	...
	for _, apipod := range apipods.Items {
		cmd, _ := exec.Command("oc", "exec", "-it", apipod.GetName(), "-n", "openshift-apiserver", "-c", "openshift-apiserver", "--", "curl", "-k", "https://localhost:8443/readyz").Output()
	...
	for _, kubepod := range kubepods.Items {
		cmd, _ := exec.Command("oc", "exec", "-it", kubepod.GetName(), "-n", "openshift-kube-apiserver", "-c", "kube-apiserver", "--", "curl", "-k", "https://localhost:6443/readyz").Output()
